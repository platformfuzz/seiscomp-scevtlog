# Ubuntu 24.04 + public gsm. scevtlog only. Pin SEISCOMP_VERSION for replayable builds.
...
COPY docker/entrypoint.sh docker/run-processor.sh /docker/

RUN chmod 0755 /docker/entrypoint.sh /docker/run-processor.sh
